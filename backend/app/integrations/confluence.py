    ...
    try:
        ...
        for i, step_text in enumerate(steps):
            ...
            if is_decision:
                ...
                draw.rounded_rectangle([x1, y1, x2, y2], radius=8, fill=fill_color, outline='#2563eb', width=3)
            ...
            if i < len(steps) - 1:
                ...
                draw.line([(x_center, arrow_start_y), (x_center, arrow_end_y)], fill='#2563eb', width=4)
                # Arrow head
                draw.polygon([
...
def replace_mermaid_with_image(html: str, image_filename: str) -> str:
    ...
    pattern = r'<pre><code class="language-mermaid">[\s\S]*?</code></pre>'
    replacement = f'<ac:image ac:align="center" ac:layout="center"><ri:attachment ri:filename="{image_filename}"/></ac:image>'
...
def publish_to_confluence(title: str, html: str) -> Optional[str]:
    ...
    # If we have a diagram image, we'll temporarily use a placeholder
